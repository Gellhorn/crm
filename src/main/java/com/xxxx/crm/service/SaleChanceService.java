...
public class SaleChanceService extends BaseService<SaleChance, Integer> {
    ...
    public void addSaleChance(SaleChance saleChance) {
        ...
        /* 2. 设置相关字段的默认值 */
        // isValid是否有效  （0=无效，1=有效） 设置为默认有效 1= 有效
    ...
    public void updateSaleChance(SaleChance saleChance) {
        ...
        // 通过主键查询对象
        SaleChance originSaleChance = saleChanceMapper.selectByPrimaryKey(saleChance.getId());
        // 判断数据库中对应的记录存在
        AssertUtil.isTrue(originSaleChance == null,"待更新记录不存在！");
        // 参数校验
        checkSaleChanceParams(saleChance.getCustomerName(),saleChance.getLinkMan(), saleChance.getLinkPhone());
        ...
        // assignMan指派人
        // 判断原始数据是否存在
        if (StringUtils.isBlank(originSaleChance.getAssignMan())) { // 不存在
            // 判断修改后的值是否存在
